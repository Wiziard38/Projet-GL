...
# Script pour tester tous les tests de syntaxes qui sont valides
RED='\033[0;31m'
GREEN='\033[0;32m'
NC='\033[0m'
...
input_dir="$source_dir/src/test/deca/syntax/valid/homemade"
...
test_synt_valide () {
    ...
    if echo "$test_result" | grep -q -e "$1:[0-9][0-9]*:"; then
        printf "\033[1A"
        echo -e "${RED}Echec inattendu pour test_synt sur ${NC}$1"
        echo ""
        echo ""
        ...
        printf "\033[1A"
        echo -e "${RED}Erreur non soulevée pour ${NC}$1"
        echo ""
        echo ""
        total_failed=$((total_failed+1))
